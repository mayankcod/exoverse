...
introOverlay.style.background = 'back.jpg';
...
introOverlay.style.fontSize = '24px';
introOverlay.style.zIndex = 10; // Ensure it's above the canvas
introOverlay.innerHTML = `
  <div>
    <h1>Welcome to the Exoplanet Visualizer</h1>
    <p>Discover distant worlds and their secrets</p>
    <button id="start-btn" style="margin-top: 20px; padding: 10px 20px; font-size: 18px; cursor: pointer;">
      Start Exploring
    </button>
  </div>
`;
...
document.getElementById('start-btn').addEventListener('click', () => {
  ...
  setTimeout(() => {
    introOverlay.style.display = 'none'; // Hide it completely after the fade-out
    startScene(); // Start the Three.js scene
...
sunLight.position.set(2, 2, 2); // Higher and more central position
sunLight.castShadow = true; // Enable shadows if required
sunLight.shadow.mapSize.width = 1024; // Higher quality shadow maps
sunLight.shadow.mapSize.height = 1024;
sunLight.shadow.camera.near = 0.5;
sunLight.shadow.camera.far = 500;
scene.add(sunLight);

// Hemisphere Light (Ambient-like soft light)
const hemisphereLight = new THREE.HemisphereLight(0xffffff, 0x444444, 1.0); 
hemisphereLight.position.set(0, 100, 0); // Above the scene
scene.add(hemisphereLight);

// Point Light (Camera-following light for close-ups)
const pointLight = new THREE.PointLight(0xffffff, 1.5, 200);
pointLight.position.copy(camera.position); // Place at the camera's position
scene.add(pointLight);

// Update Point Light Position During Animation
function updateLights() {
  pointLight.position.copy(activeCamera.position); // Keep light near the current camera
}
